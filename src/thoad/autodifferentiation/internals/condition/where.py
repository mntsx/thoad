...
class WhereXBackward0(ContractiveFunction):
    ...
    def check_shape(self, shape: Shape, indep: Indep) -> Tuple[Shape, Indep]:
        ...
        projected_shape: Shape = tuple(saved_condition.shape)
    ...
    def _extract_context(self) -> None:
        # extract info
        saved_condition: Tensor = self._grad_fn._saved_condition
        saved_condition.to(dtype=self._dtype, device=self._device)
        # save context
    ...
    def _process_context(self) -> None:
        ...
        assert self._context is not None
        # load context
        saved_condition: float = self._context["saved_condition"]
        # process context
        # ...
        # save processed context
        processed_context: dict[str, Any] = dict()
        processed_context["condition"] = saved_condition
    ...
    def _compute_internal_0_1(self) -> None:
        ...
        ### Instantiate differential
        differential: Tensor
        differential = torch.where(condition=condition, input=t0, other=t1)
