...
class SelectXBackward0(ContractiveFunction):
    ...
    def check_shape(
        ...
        projected_shape: Shape = tuple(
            sz for d, sz in enumerate(input_shape) if d != dim
        )
    ...
    def _extract_context(self) -> None:
        ...
        saved_index: int = getattr(self._grad_fn, "_saved_index")
    ...
    def _process_context(self) -> None:
        ...
        saved_index: int = self._context["saved_index"]
        ...
        index: int = recalibrate_index(
            index=saved_index, dim_size=saved_self_sym_sizes[dim]
        )
        input_shape: Shape = denull_shape(shape=saved_self_sym_sizes)
        # save processed context
        processed_context: dict[str, Any] = dict()
        processed_context["dim"] = dim
        processed_context["index"] = index
    ...
    def _compute_internal_0_0(self) -> IDData:
        ...
        ### Read context
        dim: int = self._processed_context["dim"]
        index: int = self._processed_context["index"]
        ...
        external_indices: list[int] = [d for d in composed_indices if d != dim]
        # tensorize the index
        idx: Tensor = torch.tensor(data=[index], dtype=torch.long)

        ### Instantiate derivative
        derivative: Tensor
        internal_indices: list[int] = [dim]
        internal_shape: Tuple[int, ...] = (input_shape[dim],)
        derivative = F.one_hot(input=idx, num_classes=input_shape[dim])
        ...
        einstein_notation.append((tuple(internal_shape), (True,)))
