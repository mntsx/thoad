...
class AddmmXBackward0(ContractiveFunction):
    ...
    def _extract_context(self) -> None:
        ...
        saved_mat1: Tensor = getattr(self._grad_fn, "_saved_mat1")
        saved_mat1_sym_sizes: Tuple[int, ...] = getattr(
            self._grad_fn, "_saved_mat1_sym_sizes"
        )
        saved_mat1_sym_strides: Tuple[int, ...] = getattr(
            self._grad_fn, "_saved_mat1_sym_strides"
        )
        ...
        if saved_mat1 is not None:
            ...
            getattr(saved_mat1, "_fix_weakref")()
    ...
    def _process_context(self) -> None:
        ...
        m1: Tensor = denull_tensor(
            tensor=saved_mat1, dtype=self._dtype, device=self._device
        )
        m1_shape: Shape = denull_shape(shape=saved_mat1_sym_sizes)
    ...
    def _compute_internal_0_0(self) -> IDData:
        ...
        internal_independencies: list[bool]
        if beta == 1.0:
            internal_indices = list()
            internal_shape = list()
            internal_independencies = list()
            derivative: Tensor = t1.sum()
        else:
            internal_indices = [0, 1]
            internal_shape = [m1_shape[0], m2_shape[1]]
            internal_independencies = [True, True]
            derivative: Tensor = beta * t1.view(size=(1, 1))

        ### Create einstein notation
        einstein_external: Tuple[int, ...] = (0, 1)
    ...
    def _compute_internal_0_12(self) -> IDData:
        assert self._shape0 is not None
        assert self._processed_context is not None
        ### Read context
        m1_shape: Shape = self._processed_context["m1_shape"]
    ...
    def _compute_internal_0_21(self) -> IDData:
        assert self._shape0 is not None
        assert self._processed_context is not None
        ### Read context
        m1_shape: Shape = self._processed_context["m1_shape"]
    ...
    def compute_internal(self, out_id: int, inp_id: Tuple[int, ...]) -> IDData:
        ...
        match (out_id, inp_id):
            ...
            case (
                ...
                ID_data = self._compute_internal_0_12()
            case (0, (2, 1)):
                ID_data = self._compute_internal_0_21()
